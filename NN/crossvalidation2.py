...
def run_stratified_cross_validation(X, y, weights, n_splits=5, epochs=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_arr, y_arr), 1):
        ...
        print(f"Pli {fold} Terminé | Seuil Optimal: {best_t:.2f} | Signification: {max_sig:.3f} sigma")
        
    # --- BILAN DE LA VALIDATION CROISÉE ---
    print("\n" + "=" * 60)
    print("RÉSULTATS FINAUX DE LA VALIDATION CROISÉE")
    print("=" * 60)
    print(f"Signification Moyenne : {np.mean(fold_significances):.3f} ± {np.std(fold_significances):.3f} sigma")
...
def load_and_clean_blackswan():
    ...
    feature_mapping = {
        "PRI_met": "PRI_met", "PRI_met_phi": "PRI_met_phi",
        "PRI_lep_pt": "PRI_lep_pt", "PRI_lep_phi": "PRI_lep_phi",
        "PRI_had_pt": "PRI_had_pt", "PRI_had_phi": "PRI_had_phi"
    }
    
    available_features = [col for col in feature_mapping.keys() if col in df_raw.columns]
    features = df_raw[available_features].copy() if available_features else df_raw.drop(columns=['labels', 'detailed_labels', 'weights'], errors='ignore')
...
if __name__ == "__main__":
    ...
    scores_cv = run_stratified_cross_validation(features, target, weights, n_splits=5, epochs=10)