...
def test1():
    ...
    print(
        f"Counting : mu={compute_mu(score, weight, saved)['mu_hat']:.3f} ± {compute_mu(score, weight, saved)['del_mu_stat']:.3f}"
    )
    print(
        f"Likelihood : mu={compute_mub(score, weight, saved)['mu_hat']:.3f} ± {compute_mub(score, weight, saved)['del_mu_stat']:.3f}"
    )
    print(
        f"Shape : mu={compute_mu_shape(saved_shape)['mu_hat']:.3f} ± {compute_mu_shape(saved_shape)['del_mu_stat']:.3f}"
    )
...
def test3():
    ...
    holdout = {
        "data": score_holdout,
        "labels": label_holdout,
        "weights": weight_holdout,
    }
    ...
    print(
        f"Counting : mu={compute_mu(score_test, weight_test, saved)['mu_hat']:.3f} ± {compute_mu(score_test, weight_test, saved)['del_mu_stat']:.3f}"
    )
    print(
        f"Likelihood : mu={compute_mub(score_test, weight_test, saved)['mu_hat']:.3f} ± {compute_mub(score_test, weight_test, saved)['del_mu_stat']:.3f}"
    )
