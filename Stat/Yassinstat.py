...
def plot_likelihood_profile(saved_info):
    ...
    csv_scores = np.loadtxt(
        "/Users/yassin_knzidr/Documents/GitHub/Higgs-TeamB/Stat/predictions 1.csv"
    )
    ...
    plt.plot(
        mu_vals,
        delta_nll,
        label=r"$-2 \Delta \ln \mathcal{L}(\mu)$",
        color="blue",
        lw=2,
    )
    ...
    plt.axhline(
        1.0, color="red", linestyle="--", label=r"$1\sigma$ interval ($\Delta = 1$)"
    )
    plt.axvline(
        mu_hat,
        color="gray",
        linestyle=":",
        label=rf"Best fit $\hat{{\mu}} \approx {mu_hat:.2f}$",
    )
...
dummy_saved_info = {"beta": 200.0, "gamma": 5.0}
